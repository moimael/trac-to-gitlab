...
database = PostgresqlDatabase('gitlabhq_production', **{'host': '/var/opt/gitlab/postgresql', 'user': 'gitlab-psql'})
...
class CiCommits(BaseModel):
    ...
    class Meta:
        db_table = 'ci_commits'
        indexes = (
            (('gl_project', 'sha'), False),
            (('status', 'gl_project'), False),
        )
...
class MergeRequestMetrics(BaseModel):
    ...
    pipeline = ForeignKeyField(db_column='pipeline_id', null=True, rel_model=CiCommits, to_field='id')
    updated_at = DateTimeField()
...
class Users(BaseModel):
    ...
    authorized_projects_populated = BooleanField(null=True)
    avatar = CharField(null=True)
